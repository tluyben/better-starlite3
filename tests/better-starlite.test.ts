...
describe("better-starlite driver", () => {
  let db: DatabaseClient;

  before(async () => {
    db = await open({ driver: "better-starlite", filename: ":memory:" });
  ...
  it("returns standalone metadata", async () => {
    ...
    assert.equal(res.node_id, "better-starlite");
  ...
  it("driver field is set correctly", async () => {
    assert.equal(db.driver, "better-starlite");
  });
